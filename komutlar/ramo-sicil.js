...
moment.locale("tr");
sicil = sicil.reverse();
let sicilPanel = sicil.length > 0 ? sicil.map((value, index) => `\`${index + 1}.\` [**${value.Ceza}**] \`${value.Tarih}\` **tarihinde** \`${value.Sebep}\` **sebebinden dolayı** \`${value.Süre}\` **süresince** ${message.guild.members.cache.has(value.Yetkili) ? message.guild.members.cache.get(value.Yetkili) : value.Yetkili} **tarafından cezalandırıldı.**`).join("\n\n") : "**Bu Kullanıcının Sicili Kalbin Kadar Temiz!**";
message.react('✅')

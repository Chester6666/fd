...
if (!member) return message.channel.send(new MessageEmbed().setColor('000000').setDescription(`${message.author}**, lütfen bir kullanıcı etiketle !**`))
          
let mute = message.mentions.members.first() || message.guild.members.cache.find(r => r.id === args[0]);
if (!mute) { new MessageEmbed().setColor('000000').setDescription(`${message.author}**, lütfen mute atmam gereken kullanıcı belirt !**`);
} else {
if (mute.roles.highest.position >= message.member.roles.highest.position) 
              {
return message.channel.send(new MessageEmbed().setColor('000000').setDescription(`**Bu Kullanıcı Senden Üst/Aynı Pozisyonda.**`));
} else {
let zaman = args[1]   
.replace("sn", "s")
.replace("dk", "m")
.replace("sa", "h")
.replace("gün", "d");
if (!zaman) { message.channel.send(new MessageEmbed().setColor('000000').setDescription(`**Lütfen Bir zaman dilimi belirtin.**`));
} else {
let sebep = args[2]
if(!sebep) return message.channel.send(new MessageEmbed().setColor('000000').setDescription(`**Lütfen Bir sebep belirtiniz.**`))  
...
message.react('✅')          
message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('000000').setTimestamp().setDescription(`${message.author} **tarafından** ${member} \`${sebep}\` **sebebiyle** \`${zamandilimi}\` **boyunca mute atıldı**`));
mutelog.send(
new MessageEmbed()
